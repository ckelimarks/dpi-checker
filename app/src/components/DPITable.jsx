        ...
        <table className="w-full border-2 border-slate-900">
          <thead>
            <tr className="bg-slate-950 text-slate-100">
              ...
              <th className="text-left py-4 px-6 font-bold text-sm tracking-wide font-technical border-r border-slate-700">
                REQUIRED PIXELS<br />
                <span className="text-xs text-slate-400 font-normal">(for 300 DPI)</span>
              </th>
              <th className="text-left py-4 px-6 font-bold text-sm tracking-wide font-technical border-r border-slate-700">
                EFFECTIVE DPI
              </th>
          ...
          <tbody>
            {dpiResults.map((result, index) => {
              // Calculate required pixels for 300 DPI at this print size
              const requiredWidth = Math.round(result.width * 300)
              const requiredHeight = Math.round(result.height * 300)
              ...
              return (
                ...
                >
                  ...
                  <td className="py-4 px-6 font-technical text-slate-900 border-r border-slate-200">
                    <span className="inline-flex items-center gap-1">
                      <span className="font-bold">{requiredWidth}×{requiredHeight}</span>
